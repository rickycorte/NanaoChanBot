...
LIBS = -Ilib -pthread -static-libgcc -static-libstdc++ -L/usr/local/lib -lrt -lssl -lcrypto -lsasl2
FILES = Defines.h Helper.h Helper.cpp Log.h Log.cpp Bot/Update.h Bot/Update.cpp Bot/Stats.h Bot/Stats.cpp Bot/TelegramBot.h Bot/TelegramBot.cpp Bot/AnimeResultParse.h Bot/AnimeResultParse.cpp Bot/Connection.h Bot/Connection.cpp Bot/RateLimiter.h Bot/RateLimiter.cpp DataMap.h DataMap.cpp ExecutionTimer.h ExecutionTimer.cpp Source.cpp
...
all:
	echo Compiling:
	mkdir -p $(BUILD_PATH)
	$(CC) $(CFLAGS) $(SOURCES) $(LIBS) -o $(BUILD_PATH)/$(NAME)
